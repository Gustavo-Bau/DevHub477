...
export async function POST(request: Request) {
  try {
    ...
    if (existing) {
      const existingWithItems = await prisma.order.findUnique({ where: { id: existing.id }, include: { items: true } });
      return NextResponse.json({ orderId: existing.id, alreadyProcessed: true, items: existingWithItems?.items ?? [] });
    }
