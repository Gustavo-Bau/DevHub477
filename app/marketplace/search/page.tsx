...
export default function MarketplaceSearchPage() {
  ...
  const [sort, setSort] = useState<'price' | 'rating' | 'popularity'>('popularity');
    ...
    <main style={{ maxWidth: 1000, margin: '2rem auto', display: 'grid', gap: 16 }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <input
            type="checkbox"
            checked={freelancerAvailability}
            onChange={(e) => setFreelancerAvailability(e.target.checked)}
          />
          Freelancer available
        </label>
        <select value={sort} onChange={(e) => setSort(e.target.value as 'price' | 'rating' | 'popularity')}>
      ...
      <section style={{ display: 'grid', gap: 12 }}>
        {items.map((item) => (
          <article key={item.id} style={{ border: '1px solid #ddd', padding: 12 }}>
            ...
            <p>
              <strong>Type:</strong> {item.owner.role === 'freelancer' ? 'Service' : 'Product'}
            </p>
            <p>
              <strong>Category:</strong> {item.category} | <strong>Price:</strong> ${item.price.toFixed(2)}
            </p>
            <p>
              <strong>Rating:</strong> {item.ratings ?? 'n/a'} | <strong>Popularity:</strong> {item.popularity}
            </p>
            <p>
              <strong>Stack:</strong> {item.technologyStack.join(', ') || 'n/a'}
            </p>
            <p>
              <strong>Freelancer availability:</strong> {item.freelancerAvailable ? 'Available' : 'Not available'}
            </p>
            <AddToCartButton
              item={{
                id: item.id,
                ...
                type: item.owner.role === 'freelancer' ? 'service' : 'product',
              }}
            />
          </article>
        ))}
